...
export async function pickNvimPane(
	ctx: ExtensionCommandContext | ExtensionContext,
): Promise<Pane | undefined> {
	const cwdRoot = gitRoot(ctx.cwd);
	const panes = getNvimPanes().sort(
		(a, b) => scorePane(b, ctx.cwd, cwdRoot) - scorePane(a, ctx.cwd, cwdRoot),
	);
	if (panes.length === 0) {
	...
	const tied = panes.filter(
		(pane) => scorePane(pane, ctx.cwd, cwdRoot) === bestScore,
	);
...
export function openFilesInNvim(
	ctx: ExtensionCommandContext | ExtensionContext,
	files: string[],
): Promise<void> {
	return pickNvimPane(ctx).then((pane) => {
		...
		ctx.ui.notify(
			`Opened ${files.length} file(s) in nvim ${pane.target}`,
			"info",
		);
	});
}
