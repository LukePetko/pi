...
export default function (pi: ExtensionAPI) {
  ...
  function resetState(): SpeedState {
    return { running: false, startedAt: 0, lastUpdateAt: 0, estimatedTokens: 0, chars: 0 };
  }
  ...
  function sessionTotals(ctx: ExtensionContext) {
    ...
    totals.usingSubscription = !!(model && (ctx as any).modelRegistry?.isUsingOAuth?.(model));
  ...
  function speedPart(): string {
    if (!state.running && state.finalTokensPerSecond == null) return "tok/s ready";

    const now = promptRunning ? Date.now() : (promptEndedAt || state.lastUpdateAt);
    const elapsedStart = promptStartedAt || state.startedAt;
    const elapsedSeconds = Math.max(0.001, (now - elapsedStart) / 1000);
    const tokens = state.exactOutputTokens ?? state.estimatedTokens;
    const tokensPerSecond = state.finalTokensPerSecond ?? tokens / elapsedSeconds;
    return `${state.running ? "⚡" : "✓"} ${n(tokensPerSecond)} tok/s · ${n(elapsedSeconds)}s`;
  }
  ...
  function statsLine(ctx: ExtensionContext): string {
    ...
    const costPart = totals.cost || totals.usingSubscription
      ? rgb("#fb923c", `${money(totals.cost)}${totals.usingSubscription ? " (sub)" : ""}`)
      : "";
    return `↑${n(totals.input, 0)}↓${n(totals.output)}${costPart ? ` ${costPart}` : ""} ${gray("·")} ${contextPart} ${gray("·")} ${speedPart()}`;
  }
  ...
  pi.on("agent_end", async (_event, ctx) => {
    ...
    promptRunning = false;
    if (state.startedAt) {
      const elapsedSeconds = Math.max(0.001, (promptEndedAt - (promptStartedAt || state.startedAt)) / 1000);
      state.finalTokensPerSecond = (state.exactOutputTokens ?? state.estimatedTokens) / elapsedSeconds;
    }
    render(ctx);
  });
  ...
  pi.on("message_end", async (event, ctx) => {
    ...
    if (typeof usage?.output === "number" && usage.output > 0) state.exactOutputTokens = usage.output;
    const elapsedSeconds = Math.max(0.001, (state.lastUpdateAt - (promptStartedAt || state.startedAt)) / 1000);
    state.finalTokensPerSecond = (state.exactOutputTokens ?? state.estimatedTokens) / elapsedSeconds;
    render(ctx);
  });
  ...
  pi.registerCommand("statusline", {
    ...
    handler: async (_args, ctx) => {
      ...
      if (!enabled) {
        ...
        ctx.ui.notify("Status line enabled", "success");
        render(ctx);
      }
    },
  });
